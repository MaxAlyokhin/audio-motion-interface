...
**Тип отсечки** — при "полном" типе звук генерируется от точки превышения отсечки до точки ниже отсечки. При типе "до пика" звук генерируется до точки наивысшей скорости движения. Пояснение: каждый жест в конце своего движения так или иначе замедляется. Это приводит к тому, что, во-первых, сложно управлять системой, а во-вторых, в режиме влияния скорости на громкость середина звука может быть громкой, а конец очень тихим. Мы можем поймать замедление движения и интерпретировать его как конец жеста. Тогда осциллятор будет обрываться на пике скорости. На практике это позволяет добиться более чётких отдельных контролируемых звуков.
