...
### Хранение состояния (настроек) интерфейса
При первом запуске приложения устанавливаются настройки по-умолчанию.

В дальнейшем, любые изменения настроек сразу же записываются на компьютер (в `localStorage`), поэтому при перезапуске ваши наработки автоматически восстанавливаются.
